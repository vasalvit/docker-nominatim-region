...
RUN echo "<?php" > /app/src/build/settings/local.php && \
    echo "@define('CONST_Postgresql_Version', '11');" >> /app/src/build/settings/local.php && \
    echo "@define('CONST_Postgis_Version', '2.5');" >> /app/src/build/settings/local.php && \
...
RUN sudo curl http://download.geofabrik.de/${REGION}-latest.osm.pbf -o /latest.osm.pbf && \
    ...
    sudo -u postgres /usr/lib/postgresql/11/bin/initdb -D /var/lib/postgresql/11/main && \
    sudo -u postgres /usr/lib/postgresql/11/bin/pg_ctl start -D /var/lib/postgresql/11/main && \
